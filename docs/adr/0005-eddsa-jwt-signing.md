...
`golang-jwt/jwt/v5` supports EdDSA natively (`jwt.SigningMethodEdDSA`, `ParseEdPrivateKeyFromPEM`/`ParseEdPublicKeyFromPEM`) — no additional dependency is required.
...
- No new runtime dependency: the JWT library already supported EdDSA.
